...
UNIT_DEF = {
    ...
    2:  {"name":"서비스 인프라 플랫폼",    "vendor":"한국정보기술 컨소시엄", "zone":"공통인프라",          "icon":"🖥️","color":"#7c3aed"},
...
def parse_bms_projects(bms):
    ...
    for it in bms.get("items",[]):
        ...
        if num:
            ...
            units[num]["budget"] += it["총예산"]/1e8
            units[num]["exec"] += it["사용금액"]/1e8
        else:
            common_budget += it["총예산"]/1e8
            common_exec += it["사용금액"]/1e8
