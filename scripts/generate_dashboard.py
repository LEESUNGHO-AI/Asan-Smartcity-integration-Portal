...
# ══════════════════════════════════════════
#  기본 데이터 (Notion 2026-02-12 기준 직접 확인값)
...
FALLBACK_CATS = [
    ...
    ("여비",             0.4,    0.8,   191.4, "⚠️ 초과집행"),
    ("연구개발비",       10.0,    0.0,     0.0, "DRT 플랫폼 미착수"),
    ("유형자산",          0.2,    0.2,    82.1, "이노베이션센터 PC"),
    ("무형자산(SW)",     90.5,   38.3,    42.3, "6개 플랫폼 개발"),
    ("건설비",           99.4,   23.2,    23.4, "7개 시설 공사"),
    ("사업비배분",       13.0,   13.0,   100.0, "✅ 간접보조 교부완료"),
]

# ★ 서비스 인프라: 예산 45.33억 / 집행 0억(미집행) / 0%
#   → Notion 원본: 집행금액 칸이 "-" (계약체결 전)
FALLBACK_PROJECTS = [
    # num, name,                              budget, executed, rate, status,      note
    (1, "유무선 네트워크 구축",               11.35,   4.15,  36.6, "✅ 완료",   "계약완료 (연장협의 중)"),
    (2, "서비스 인프라 플랫폼",          45.33,   0.00,   0.0, "🔄 진행중", "계약체결 진행중 (선급금 예정)"),
    (3, "이노베이션 센터 구축",               13.00,  11.86,  91.2, "✅ 완료",   "구축완료 (운영중)"),
    (4, "디지털 OASIS SPOT",                 35.00,   0.15,   0.4, "🔄 진행중", "부지승인 완료, 설계용역 진행"),
    (5, "SDDC Platform 구축",               27.00,   7.09,  26.2, "🔄 진행중", "서버실 구축 진행중"),
    (6, "AI 통합관제 플랫폼",                16.00,   9.29,  58.1, "🔄 진행중", "서비스 인프라에 통합 계약"),
    (7, "디지털 OASIS 정보관리",             25.00,  10.93,  43.7, "🔄 진행중", "개발 진행중"),
    (8, "DRT 수요응답형 교통",               10.00,   0.00,   0.0, "⏸️ 대기",  "설계 진행중"),
    (9, "감리용역 (신설)",                    1.60,   0.00,   0.0, "🆕 신설",  "업체선정 준비"),
]

FALLBACK_ISSUES = [
    ("🔴 긴급", "#ef4444", "예산 집행률 저조 (40.8%) — 잔여 142억, 월평균 14.2억 집행 필요",     "142억원"),
    ("🔴 긴급", "#ef4444", "OASIS SPOT 공사 착공 지연 (집행률 0.4%)",                           "35억원"),
    ("🟠 높음", "#f59e0b", "서비스 인프라 계약 체결  — 45.33억원, 선급금 지급",     "45.33억원"),
    ("🟠 높음", "#f59e0b", "SDDC Platform 서버실 일정 조정 필요 (현 26.2% 집행)",               "27억원"),
    ("🟠 높음", "#f59e0b", "여비 초과집행 (191.4%) — 비목 간 전용 또는 실시계획 변경 필요",     "-"),
    ("🟡 주의", "#eab308", "DRT 차량 발주 설계 지연 (연구개발비 전액 미집행)",                  "10억원"),
    ("🟡 주의", "#eab308", "감리용역 업체선정 지연",                                            "1.6억원"),
    ("🟡 주의", "#eab308", "부스제작비 초과집행 (184.1%) 정리 필요",                            "-"),
]

FALLBACK_TIMELINE = [
    ("02/03", "회의",   "서버실 구축 및 장비 설치 조정 회의",           True),
    ("02/06", "제출",   "2026년 1월 관리카드 제출",                    True),
    ("02/10", "제출",   "25년 4분기·연간보고서 제출",                  True),
    ("02/10", "공문",   "국고보조금 집행실태 점검결과 소명자료 제출",   True),
    ("02/10", "공문",   "실시계획 변경 승인 요청 공문 → 아산시",       True),
    ("02/11", "협의",   "유무선인프라 구축 계약 연장 협의",             False),
    ("02/12", "동기화", "Slack #플랜예산 → Notion 전체 동기화",        True),
    ("02/28", "마감",   "2월 월간 추진실적 보고",                      None),
]
...
def collect():
    ...
    source   = "기본 데이터 (Notion 2026-02-12)"
